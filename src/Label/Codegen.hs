...
getter field =
  ...
  case1 i con =
    case con of
      ...
      GadtC    {}   -> fclError "CodeGen.getter.case1: GadtC not supported yet"
      RecGadtC {}   -> fclError "CodeGen.getter.case1: RecGadtC not supported yet"
...
modifier field =
  ...
  update i con =
    case con of
      ...
      GadtC    {}   -> fclError "CodeGen.modifier.update: GadtC not supported yet"
      RecGadtC {}   -> fclError "CodeGen.modifier.update: RecGadtC not supported yet"
  ...
  keep con =
    case con of
      NormalC c fs  -> let s = take (length fs) in (conP c (s pats), apps (conE c) (s vars))
      RecC    c fs  -> let s = take (length fs) in (conP c (s pats), apps (conE c) (s vars))
      GadtC    {}   -> fclError "CodeGen.modifier.keep: GadtC not supported yet"
      RecGadtC {}   -> fclError "CodeGen.modifier.keep: RecGadtC not supported yet"
      InfixC  _ c _ -> ( infixP (pats !! 0) c (pats !! 1)
                       , apps (conE c) (take 2 vars)
                       )
  ...
  fail_ con =
    case con of
      ...
      GadtC    {}   -> fclError "CodeGen.modifier.fail_: GadtC not supported yet"
      RecGadtC {}   -> fclError "CodeGen.modifier.fail_: RecGadtC not supported yet"
      InfixC  _ c _ -> infixP (pats !! 0) c (pats !! 1)
...
mapTypeVariables f = go
  ...
  go ty =
    case ty of
      ForallT ts a b -> ForallT (mapTyVarBndr f <$> ts) (mapTypeVariables f <$> a) (go b)
