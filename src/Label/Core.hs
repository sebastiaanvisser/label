...
(>-) :: Monad m
     ...
     -> Label m m (f -> g) (o -> i)
     -> View m f g j o
(>-) (Ops f _) (Ops g m) = View (make g (\n -> m (f <=< n)))
(>-) (Ops f _) Id        = View (make return (f <=<))
(>-) Id        l         = View l
...
-- | Turn a isomorphism into a label in a singular context.

iso :: Monad m => Iso m m f o -> Iso m m g i -> Label m m (f -> g) (o -> i)
iso (Iso f _) (Iso _ b) = make f (\m -> b <=< m <=< f)

-- | Turn a isomorphism into a monomorphic label.

isom :: Monad n => Iso m m g o -> Iso n n g o -> Label m n (g -> g) (o -> o)
isom (Iso h _) (Iso f b) = make h (\m -> b <=< m <=< f)
